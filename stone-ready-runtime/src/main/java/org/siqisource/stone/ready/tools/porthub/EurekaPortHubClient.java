package org.siqisource.stone.ready.tools.porthub;
...
public class EurekaPortHubClient extends PortHubClient implements ApplicationListener<WebServerInitializedEvent> {
	...
	public void onApplicationEvent(WebServerInitializedEvent event) {
		Map<String, IRule> as = event.getApplicationContext().getBeansOfType(IRule.class);
		this.serverPort = event.getWebServer().getPort();
		if (!this.enabled || StringUtils.isEmpty(this.ip) || this.port == 0 || StringUtils.isEmpty(this.account)
